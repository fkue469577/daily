function Stream(head,tailPromise){if(typeof head=='undefined'){head=null;}
else if(typeof tailPromise=='undefined'){tailPromise=function(){return new Stream();};}
    this.headValue=head;this.tailPromise=tailPromise;}
Stream.prototype={empty:function(){return this.headValue===null;},head:function(){if(this.empty()){throw'Cannot get the head of the empty stream.';}
        ...
        var self=this;return new Stream(this.head(),function(){return self.tail().take(howmany-1);});},print:function(n){var target;if(typeof n!='undefined'){target=this.take(n);}
    else{target=this;}
