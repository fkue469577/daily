...
function removeRole(menuId, callback) {
	$.postJson("/system/manager/role/removeMenuRelation", {roleId: roleId, menuId: menuId}, function (result) {
		callback(result);
	});
}
function addRole(menuId, callback) {
	$.postJson("/system/manager/role/addMenuRelation", {roleId: roleId, menuId: menuId}, function (result) {
		callback(result);
	});
}
function removePermission(permissionId, callback) {
	$.postJson("/system/manager/role/removePermissionRelation", {roleId: roleId, permissionId: permissionId}, function (result) {
		callback(result);
	})
}
function addPermission(permissionId, callback) {
	$.postJson("/system/manager/role/addPermissionRelation", {roleId: roleId, permissionId: permissionId}, function (result) {
		callback(result);
	})
}
...
$(document).on("click", ".edit-button", function () {
	var _this = $(this);
	$(_this.parent()).checkCommit({url: "/system/manager/role/save"})
})
...
function loadMenuAndPermission(roleId, tips) {
	$.get("/system/manager/role/listMenuExpendRole/"+roleId, {}, function (result) {
...
$(document).on("click", ".del", function () {
	...
	layer.confirm("是否删除该角色?", function () {
		$.get("/system/manager/role/remove/" + _this.parents(".jslb").attr("data-id"), function (result) {
			refresh(result)
		});
	})
})
...
function openRole(role) {
	layer.open({
		...
		, btn: ['提交', '取消']
		, yes: function () {
			$("#addForm").checkCommit({url: "/system/manager/role/insert"})
		}
	});
}
